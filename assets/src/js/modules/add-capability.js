 ...
 * @returns void
 */
export function addNewCapability(
	inputSelector,
	addBtnSelector,
	listSelector
) {
	...
	addBtn.addEventListener( 'click', function () {
		...
		capabilitiesList.appendChild(
			createCapabilityItem( input.value.trim(), name, capabilitiesList )
		);
		input.value = '';
	} );
}
...
export function addCapabilityFromList(
	checkboxSelector,
	...
	submitButton.addEventListener( 'click', ( e ) => {
		...
		checked.forEach( ( checkbox ) => {
			...
			list.appendChild(
				createCapabilityItem( capname, checkbox.name, list )
			);
